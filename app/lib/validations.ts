...
const baseRsvpSchema = z
  .object({
    accommodation: z
      .enum(
        [
          AccommodationType.ROOF,
          AccommodationType.OWN_TENT,
          AccommodationType.NO_SLEEP,
        ],
        {
    ...
    childrenCount: z
      .number()
      .min(0, "Počet dětí musí být alespoň 0")
      .max(20, "Maximálně 20 dětí"),
    ...
    petsCount: z
      .number()
      .min(0, "Počet zvířátek musí být alespoň 0")
      .max(20, "Maximálně 20 zvířátek"),
  })
  .refine(
    (data) => {
  ...
  .refine(
    (data) => {
      if (
        data.attending === "yes" &&
        data.drinkChoice === "other" &&
        !data.customDrink?.trim()
      ) {
        return false;
      }
      return true;
...
export const rsvpSchema = baseRsvpSchema.extend({
  ...
  names: z
    .array(z.string().min(2, "Jméno musí mít alespoň 2 znaky"))
